...
static void _initAvatarShader() {
	// Compile the reference shaders
	_skinnedMeshProgram = LoadShaders("AvatarVertexShader.glsl", "AvatarFragmentShader.glsl");
	_skinnedMeshPBSProgram = LoadShaders("AvatarVertexShader.glsl", "AvatarFragmentShaderPBS.glsl");
	_combinedMeshProgram = LoadShaders("AvatarVertexShader.glsl", "AvatarFragmentShader_CombinedMesh.glsl");

}

// update avatar status in rift every frame
static void _runAvatar(
	float deltaSeconds,
	const ovrAvatarTransform& hmd,
	const ovrAvatarHandInputState& left,
	const ovrAvatarHandInputState& right,
	...
	_updateAvatar(_avatar, deltaSeconds, hmd, left, right, packet, packetPlaybackTime);
}
